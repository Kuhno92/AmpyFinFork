...
from config import POLYGON_API_KEY, FINANCIAL_PREP_API_KEY, MONGO_DB_USER, MONGO_DB_PASS, API_KEY, API_SECRET, BASE_URL, \
    mongo_url
from helper_files.signal_logger import TradingSignalLogger
...
tradingSignalLogger = TradingSignalLogger(mongo_url)
...
def log_signal(mongo_client, strategy_name, signal_type, details=None):
    """
    ...
    mongo_client.signals_collection.insert_one(signal)
...
def simulate_trade(ticker, strategy, historical_data, current_price, account_cash, portfolio_qty, total_portfolio_value, mongo_client):
   ...
   if action in ["buy", "sell"]:
       tradingSignalLogger.log_signal(strategy_name=strategy.__name__, ticker=ticker, action=action, details={
   ...
   while True:
      ...
      if status == "open":
         ...
         for ticker in ndaq_tickers:
            #process_ticker(ticker, mongo_client, stock_client)
